...
void add_friend(int id1, int id2) {
    ...
    if (user1 && user2) {
        if (user1->friend_count < MAX_FRIENDS && user2->friend_count < MAX_FRIENDS) {
            // Arkadaşın zaten eklenmemiş olduğundan emin ol
        ...
        printf("Kullanicilardan biri veya ikisi bulunamadı.\n");
    }
}
...
void dfs(int current_id, int target_depth, int current_depth, int visited[], int start_id) {
    ...
    if (current_depth == target_depth) {
        if (current_id != start_id) { // Kendisini listeleme
            printf("%d ", current_id);
        }
        return;
    }
...
int i;
    for ( i = 0; i < user->friend_count; i++) {
        ...
        if (friend_id < MAX_USERS && !visited[friend_id]) { // Geçerli ID aralığı kontrolü
    ...
    for ( i = 0; i < user1->friend_count; i++) {
        for (j = 0; j < user2->friend_count; j++) {
            if (user1->friends[i] == user2->friends[j]) {
                ...
                found = 1;
                break; // Bir ortak arkadaş bulunduğunda iç döngüden çıkılabilir
            }
        }
    }
    if (!found) {
...
int i;
    for ( i = 0; i < user_count; i++) {
        ...
        if (id < MAX_USERS && !visited[id]) { // Geçerli ID aralığı kontrolü
            printf("Topluluk %d: ", community_count + 1);
            dfs_print_community(id, visited); // Yeni fonksiyon: topluluğu yazdır
            community_count++;
            printf("\n");
        }
    }
...
// Topluluk elemanlarını yazdırmak için yardımcı DFS
void dfs_print_community(int current_id, int visited[]) {
    visited[current_id] = 1;
    printf("%d ", current_id);
...
int i;
    for ( i = 0; i < user->friend_count; i++) {
        int friend_id = user->friends[i];
        if (friend_id < MAX_USERS && !visited[friend_id]) { // Geçerli ID aralığı kontrolü
            dfs_print_community(friend_id, visited);
        }
    }
}
...
// Yeni düğüm oluştur
RBNode* create_node(int user_id) {
    RBNode* new_node = (RBNode*)malloc(sizeof(RBNode));
    new_node->user_id = user_id;
    new_node->color = RED; // Yeni düğümler başlangıçta kırmızıdır
...
void insert_fixup(RBNode* z) {
    while (z->parent != NULL && z->parent->color == RED) {
        if (z->parent == z->parent->parent->left) {
            ...
            if (y != NULL && y->color == RED) { // Durum 1
                z->parent->color = BLACK;
                y->color = BLACK;
                z->parent->parent->color = RED;
                z = z->parent->parent;
            } else {
                if (z == z->parent->right) { // Durum 2
                    z = z->parent;
                    left_rotate(z);
                }
                // Durum 3
...
// Kullanıcı ID ekleme (Red-Black Tree)
void rb_insert(int user_id) {
    RBNode* z = create_node(user_id);
    RBNode* y = NULL;
    RBNode* x = root;
    ...
    while (x != NULL) {
        ...
        if (z->user_id < x->user_id)
            x = x->left;
        else if (z->user_id > x->user_id) // Eşitlik durumunda ekleme yapma
            x = x->right;
        else
            return; // ID zaten var, ekleme yapma
    }
    z->parent = y;
    if (y == NULL)
        root = z;
    else if (z->user_id < y->user_id)
        y->left = z;
    else
        y->right = z;
...
RBNode* rb_search(RBNode* node, int user_id) {
    if (node == NULL || node->user_id == user_id)
        return node;

    if (user_id < node->user_id)
        return rb_search(node->left, user_id);
    else
        return rb_search(node->right, user_id);
}
...
int i;
    for ( i = 0; i < user_count; i++) {
        int id = users[i].user_id;
        if (id < MAX_USERS) { // Geçerli ID aralığı kontrolü
            int visited[MAX_USERS] = {0};
            int count = 0;

            dfs_influence(id, visited, &count);
            count--; // kendisini saymayalım
...
int main() {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (sscanf(line, "%s %d %d", command, &id1, &id2) >= 2) {
            if (strcmp(command, "USER") == 0) {
                add_user(id1);
                rb_insert(id1); // Kullanıcı oluşturulurken Red-Black Tree'ye ekle
            ...
            if (strcmp(command, "USER") == 0) {
                add_user(id1);
                rb_insert(id1); // Tek ID'li USER komutu için
            }
        }
    }
    fclose(file);
    ...
    int search_id = 104;
    RBNode* found = rb_search(root, search_id);
    if (found != NULL)
        printf("\nKullanici %d Red-Black Tree'de bulundu.\n", search_id);
    else
        printf("\nKullanici %d Red-Black Tree'de bulunamadi.\n", search_id);
